...
export default function AddCaptureModal({ isOpen, onOpenChange }: AddCaptureModalProps) {
  ...
  if (isOpen && !lastOpenState) {
    setFormData(getDefaultFormData(selectedProgram?.id || ""));
              ...
              <Table aria-label="New capture form">
                ...
                <TableBody>
                  <TableRow key="new-capture">
                    {CAPTURE_COLUMNS.filter(col => col.key !== "actions").map((column) => {
                      const inputColor = getInputColor(column.key as keyof CaptureFormData);
                      return (
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                if (column.key !== "actions") {
                                  handleInputChange(column.key, e.target.value, column.maxLength);
                                }
                              }}
                              onKeyDown={(e) => {
                                if (column.key !== "actions") {
                                  handleKeyDown(e, column.key);
                                }
                              }}
